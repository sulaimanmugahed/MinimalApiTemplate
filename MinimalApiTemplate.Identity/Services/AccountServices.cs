...
using MinimalApiTemplate.Domain.Settings;
...
public class AccountServices(UserManager<ApplicationUser> userManager, IAuthenticatedUserService authenticatedUser, SignInManager<ApplicationUser> signInManager,IOptionsSnapshot<JWTSettings>  jwtSettings) : IAccountServices
{
    ...
    public async Task<BaseResult<AuthenticationResponse>> AuthenticateByUserName(string username)
    {
        ...
        if (user == null)
        {
            return new BaseResult<AuthenticationResponse>(new Error(ErrorCode.NotFound, "Not Found", nameof(username)));
        }
